...
public abstract class BaseFileRepositoryResource<DTO extends BaseFileRepositoryDto, ID extends Serializable, SERVICE extends BaseFileRepositoryService>
    ...
    public BaseFileRepositoryResource(Class clazz) {
        super(clazz);
    }
    ...
    public ResponseEntity<byte[]> download(@PathVariable("uid") String uid)
        ...
        LOG.debug("Fetching file with uid={}", uid);
    ...
    @RequestMapping(value = "/{id}/upload/{prefix}", method = RequestMethod.POST)
    public ResponseEntity<DTO> upload(@PathVariable("id") Long id, @PathVariable("prefix") String prefix, MultipartFile file)
        ...
        LOG.debug("Uploading image to header={} with fileName={} and prefix={}", id, file.getOriginalFilename(), prefix);
        return new ResponseEntity<>((DTO) service.uploadImage(id, prefix, file), HttpStatus.OK);
    }
}
